...
describe('validateEnvironment', () => {
	...
	describe('with valid configuration', () => {
		...
		it('should accept valid NODE_ENV values', () => {
			expect(validateEnvironment({ ...validEnv, NODE_ENV: 'development' }).NODE_ENV).toBe('development');
			expect(validateEnvironment({ ...validEnv, NODE_ENV: 'production' }).NODE_ENV).toBe('production');
	...
	describe('with invalid configuration', () => {
		it('should throw when JWT_SECRET is missing', () => {
			expect(() => validateEnvironment({ JWT_REFRESH_SECRET: 'secret' })).toThrow('Environment validation failed');
		});

		it('should throw when JWT_REFRESH_SECRET is missing', () => {
			expect(() => validateEnvironment({ JWT_SECRET: 'secret' })).toThrow('Environment validation failed');
		});

		it('should throw when JWT_SECRET is empty', () => {
			expect(() => validateEnvironment({ JWT_SECRET: '', JWT_REFRESH_SECRET: 'secret' })).toThrow('Environment validation failed');
		});

		it('should throw for invalid NODE_ENV', () => {
			expect(() => validateEnvironment({ ...validEnv, NODE_ENV: 'invalid' })).toThrow('Environment validation failed');
		});

		it('should throw for invalid PORT (negative)', () => {
			expect(() => validateEnvironment({ ...validEnv, PORT: '-1' })).toThrow('Environment validation failed');
		});

		it('should throw for invalid BCRYPT_ROUNDS (too low)', () => {
			expect(() => validateEnvironment({ ...validEnv, BCRYPT_ROUNDS: '5' })).toThrow('Environment validation failed');
		});

		it('should throw for invalid BCRYPT_ROUNDS (too high)', () => {
			expect(() => validateEnvironment({ ...validEnv, BCRYPT_ROUNDS: '25' })).toThrow('Environment validation failed');
		});

		it('should throw for invalid ADMIN_EMAIL format', () => {
			expect(() => validateEnvironment({ ...validEnv, ADMIN_EMAIL: 'not-an-email' })).toThrow('Environment validation failed');
		});
	});
